...
class GlossaryExtractionAPITests(APITestCase):
    ...
    def test_trigger_requires_ai_provider(self):
        resp = self.owner_client.post(
            f'/api/project/{self.project.pk}/glossary/extract')
    ...
    def test_trigger_creates_job(self, mock_async):
        ...
        mock_async.assert_called_once_with(
            'api.tasks.run_glossary_extraction_job', data['id'])
...
class GlossarySuggestionsAPITests(APITestCase):
    ...
    def test_get_suggestions_returns_list(self):
        suggestions = [
            {'term': 'Submit', 'definition': 'To send',
                'translations': [], 'status': 'pending'},
        ]
        make_extraction_job(self.project, user=self.owner,
                            status='complete', suggestions=suggestions)
        resp = self.owner_client.get(
            f'/api/project/{self.project.pk}/glossary/suggestions')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_accept_suggestion_creates_term(self):
        suggestions = [
            {'term': 'Submit', 'definition': 'To send', 'translations': [
                {'language_code': 'DE', 'preferred_translation': 'Absenden'}], 'status': 'pending'},
        ]
        make_extraction_job(self.project, user=self.owner,
                            status='complete', suggestions=suggestions)
        ...
        self.assertTrue(GlossaryTerm.objects.filter(
            project=self.project, term='Submit').exists())
    ...
    def test_accept_with_overrides(self):
        ...
        resp = self.owner_client.patch(
            f'/api/project/{self.project.pk}/glossary/suggestions',
            {'index': 0, 'action': 'accept',
                'term': 'OverrideTerm', 'translations': []},
            format='json'
        )
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(GlossaryTerm.objects.filter(
            project=self.project, term='OverrideTerm').exists())

    def test_accept_merges_existing_term(self):
        make_glossary_term(self.project, term='Submit', definition='Old def')
        suggestions = [
            {'term': 'Submit', 'definition': 'New def',
                'translations': [], 'status': 'pending'},
        ]
        make_extraction_job(self.project, user=self.owner,
                            status='complete', suggestions=suggestions)
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(GlossaryTerm.objects.filter(
            project=self.project, term='Submit').count(), 1)
        self.assertEqual(GlossaryTerm.objects.get(
            project=self.project, term='Submit').definition, 'New def')

    def test_reject_suggestion(self):
        suggestions = [
            {'term': 'Submit', 'definition': '',
                'translations': [], 'status': 'pending'},
        ]
        make_extraction_job(self.project, user=self.owner,
                            status='complete', suggestions=suggestions)
        ...
        self.assertFalse(GlossaryTerm.objects.filter(
            project=self.project, term='Submit').exists())
...
class RunGlossaryExtractionJobTests(TestCase):
    ...
    def test_task_sets_failed_when_no_ai_provider(self):
        job = make_extraction_job(
            self.project, user=self.owner, status='pending')
